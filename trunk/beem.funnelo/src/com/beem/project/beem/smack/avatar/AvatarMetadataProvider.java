		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if ("info".equals(parser.getName())) {
					...
					try {
						if (sbytes != null) bytes = Integer.parseInt(sbytes);
					} catch (NumberFormatException e) {}
					if (bytes != 0 && id != null && type != null) info = new AvatarMetadataExtension.Info(id, type,
							bytes);
					else // invalid info
					continue;
					...
					try {
						int height = 0;
						int width = 0;
						if (sheight != null) height = Integer.parseInt(parser.getAttributeValue(null, "height"));
						if (swidth != null) width = Integer.parseInt(parser.getAttributeValue(null, "width"));
