...
public class ChatAdapter extends IChat.Stub {
	...
	private void addMessage(Message msg) {
		if (mMessages.size() == HISTORY_MAX_SIZE) mMessages.remove(0);
		mMessages.add(msg);
	...
	public void saveHistory(Message msg, String contactName) {
		...
		File filepath;
		if (contactName.equals(msg.getFrom())) filepath = new File(path, StringUtils.parseBareAddress(contactName));
		else filepath = new File(path, StringUtils.parseBareAddress(msg.getTo()));
		path.mkdirs();
		try {
	...
	private void logMessage(com.beem.project.beem.service.Message message) {
		...
		if (mIsHistory && Environment.MEDIA_MOUNTED.equals(state)) saveHistory(message, mAccountUser);
	...
	private com.beem.project.beem.service.Message otrEncryptMessage(com.beem.project.beem.service.Message unencrypted) {
		...
		if (mOtrSessionId != null && unencrypted != null && unencrypted.getBody() != null) {
			try {
				String body = BeemOtrManager.getInstance().getOtrManager().transformSending(mOtrSessionId,
						unencrypted.getBody());
	...
	private class MsgListener implements ChatStateListener {
		...
		public void processMessage(Chat chat, org.jivesoftware.smack.packet.Message message) {
			...
			for (int i = 0; i < n; i++) {
				...
				try {
					if (listener != null) listener.processMessage(ChatAdapter.this, msg);
	...
	public boolean localEndOtrSession() throws OtrException {
		if (mOtrSessionId == null) return true;
	...
	public String getLocalOtrFingerprint() throws RemoteException {
		if (mOtrSessionId == null) return null;
	...
	public void verifyRemoteFingerprint(boolean ok) {
		if (mOtrSessionId != null) {
			if (ok) BeemOtrManager.getInstance().verifyRemoteFingerprint(mOtrSessionId);
			else BeemOtrManager.getInstance().unverifyRemoteFingerprint(mOtrSessionId);
		}
	}
